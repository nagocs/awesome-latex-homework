...
\usepackage{float} % ábrák és táblázatok elhelyezése
...
\usepackage{pdfpages} % teljes PDF-oldalak beillesztése
...
\sisetup{%
     output-decimal-marker = {,}, % tizedesvessző
}
\usepackage{icomma}
\usepackage{amsmath,amsthm,amsfonts,amssymb,amscd} % matek
\usepackage{mathtools} % kiegészítő matematikai eszközök
\usepackage{siunitx} % SI mértékegységek
...
\usepackage{csquotes} % idézetek formázása
\usepackage{booktabs} % szép táblázatok készítése
...
\frenchspacing % A mondatvégi pont utáni hosszú szünetet szünteti meg, ami idegen a magyar nyelvtől
...
% \numberwithin{equation}{section} % egyenletek számozása
% \numberwithin{figure}{section} % ábrák számozása
% \numberwithin{table}{section} % táblázatok számozása
